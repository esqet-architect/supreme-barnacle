#!/usr/bin/env python3
"""
MINERVA V7.3: Scale-Invariant Singular Value Space Characterizer
=================================================================
- Fixed: Replaced absolute covariance tolerance with scale-invariant thin SVD ratios (sigma_i / sigma_1 > epsilon).
- Preserved: Exact biorthogonal spectral projection to isolate the stable contracting plane.
- Preserved: Exact SymPy algebraic factorization for checking Q-irreducibility.
"""
...
# ============================================================================
# 2. INTRINSIC COCYCLE GENERATION
# ============================================================================
...
class IntrinsicSubstitutionSystem:
    ...
    def _characterize_system(self):
        ...
        # Enforce strict biorthogonal normalization: <w_PF, v_PF> = 1
        w_PF = w_PF / np.dot(w_PF, v_PF)

        self.v_PF = v_PF / np.sum(v_PF)  # normalized invariant token frequency
        self.w_PF = w_PF
...
# ============================================================================
# 3. BIORTHOGONAL SPECTRUM & SCALE-INVARIANT SVD RANKING
# ============================================================================
...
def exact_spectral_projector(system, walk):
    ...
    # Biorthogonal projection matrix onto the stable hyperplane
...
def compute_scale_invariant_geom_dim(points, epsilon=1e-7):
    """Computes geometric rank via relative singular value thresholds."""
    if len(points) < 2:
        return 0
    # Center the trajectory matrix explicitly
    centered_trajectory = points - np.mean(points, axis=0)
    
    # Thin SVD calculation directly on the coordinates
    svals = np.linalg.svd(centered_trajectory, full_matrices=False, compute_uv=False)
    sigma_1 = svals[0]
    
    if sigma_1 < 1e-12:
        return 0
        
    # Relative scaling ratio test removes absolute coordinate bounds
    return int(np.sum((svals / sigma_1) > epsilon))

# ============================================================================
# 4. GEOMETRIC METRICS (EMPIRICAL FINITE ESTIMATES)
# ============================================================================
...
def estimate_correlation_dimension_gp(points, num_samples=600):
    n = len(points)
    if n < 50:
        return 0.0
...
def main():
    print("=" * 90)
    print("MINERVA V7.3: INVARIANT SPECTRUM AND DIRECT MATRIX COCYCLE ANALYZER")
    print("=" * 90)

    results = []

    for name, cfg in SYSTEMS.items():
        print(f"\n📐 Running Invariant Swarm Sweep: {name}")
        print("─" * 65)
        ...
        # Apply scale-invariant projection layers
        projected, stable_dim = exact_spectral_projector(sys_obj, walk)
        geom_dim = compute_scale_invariant_geom_dim(projected)
        
        gamma = compute_wandering_exponent(projected)
        d2_est = estimate_correlation_dimension_gp(projected)
        ...
        print(f"   Stable Projector Rank:   {stable_dim}D")
        print(f"   Thin SVD Geometric Dim:  {geom_dim}D")
        print(f"   Wandering Exponent γ:    {gamma:.4f}")
        print(f"   Est. Correlation D₂:     {d2_est:.4f}")
        ...
        print(f"   Classification Profile:  {cls}")
        ...
        results.append({
            ...
            "Est_Correlation_D2": d2_est,
            "Classification": cls
        })

    print("\n" + "=" * 90)
    print("📊 MATRICIZED COCYCLE GEOMETRY DASHBOARD (V7.3)")
    print("-" * 90)
    print(f"{'System':<25} {'Pisot':<8} {'Rank':<6} {'Geom':<6} {'γ':<8} {'Est D₂':<8} {'Classification'}")
    print("-" * 90)
    for r in results:
        print(f"{r['System']:<25} {r['Strict_Pisot']:<8} {r['Projector_Rank']:<6} {r['Geom_Dim']:<6} "
              f"{r['Wandering_Gamma']:<8.4f} {r['Est_Correlation_D2']:<8.4f} {r['Classification']}")
    print("=" * 90)
