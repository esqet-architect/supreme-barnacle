#!/usr/bin/env python3
"""
minerva_v8.9_validated_cj.py
=============================
Thermodynamically Pure Chhabra-Jensen Multifractal Estimator.
Implements centered grid partitioning to stabilize grid-aliasing errors.
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# CENTERED CHHABRA-JENSEN ENGINE
# ═══════════════════════════════════════════════════════════════════════════

class CenteredChhabraJensenEstimator:
    def __init__(self, pts, weights=None, initial_scales=None):
        ...
        # Center of mass calculation for stable grid alignment
        self.center = np.mean(self.pts, axis=0)

    def _get_centered_partition(self, eps):
        """Maps coordinates relative to the center of mass to stabilize edge cutoff."""
        grid = np.floor((self.pts - self.center) / eps).astype(int)
    ...
    def compute_spectrum(self, q_values):
        ...
        for s, eps in enumerate(self.scales):
            mu = self._get_centered_partition(eps)
...
def run_validation():
    ...
    print("=" * 65)
    print("MINERVA V8.9: CENTERED GRID MOVEMENT DECOUPLING")
    print("=" * 65)

    # TEST 1: Uniform Square
    print("\nTEST 1: Uniform Grid Square (Target: D₀=2.000, Δα≈0)")
    pts1 = dataset_uniform_square(n=6400)
    # Using clean fractions to match grid alignment perfectly
    scales1 = 1.0 / np.arange(4, 18)
    est1 = CenteredChhabraJensenEstimator(pts1, initial_scales=scales1)
    ...
    est2 = CenteredChhabraJensenEstimator(pts2[:, :1], initial_scales=scales2)
    ...
    est3 = CenteredChhabraJensenEstimator(centres3, weights=weights3, initial_scales=scales3)
    ...
    if os.path.exists(rauzy_path):
        ...
        est_rauzy = CenteredChhabraJensenEstimator(pts_rauzy, initial_scales=scales_rauzy)
