#!/usr/bin/env python3
"""
minerva_v8.7_validated_cj.py
=============================
Thermodynamically Consistent Chhabra-Jensen Multifractal Estimator.
Optimized scaling windows to eliminate finite-size edge bias.
Includes production loading block for Rauzy point datasets.
"""
...
class EnsembleConsistentEstimator:
    ...
    def _get_jitter_partition(self, eps, seed):
        """Computes a unique spatial box distribution for a single isolated jitter."""
    ...
    def _find_optimal_global_window(self, log_eps, log_Zq_matrix, min_window=4):
        """Finds a single scale range maximizing linearity across all thermodynamic moments."""
    ...
    def compute_spectrum(self, q_values):
        ...
        for s, eps in enumerate(self.scales):
            ...
            for k in range(self.n_jitters):
                # Isolate the spatial partition to prevent coordinate blurring
            ...
            # Establish unrounded expectation values over the jitter ensemble
            for j in range(n_q):
...
def run_validation():
    ...
    print("=" * 65)
    print("MINERVA V8.7: FINITE-SIZE CORRECTED SCALE WINDOWS")
    print("=" * 65)

    # TEST 1: Uniform Square
    print("\nTEST 1: Uniform Grid Square (Target: D₀=2.000, Δα≈0)")
    pts1 = dataset_uniform_square(n=6400)
    scales1 = np.geomspace(0.04, 0.15, 14)  # Refined window to clip bad macro-scales
    ...
    scales2 = 1.0 / (3.0 ** np.arange(1, 6))  # Exact structural matches
    ...
    scales3 = 1.0 / (2.0 ** np.arange(3, 10))  # Shifted to avoid macroscopic scale collapsing
    ...
    if os.path.exists(rauzy_path):
        ...
        est_rauzy = EnsembleConsistentEstimator(pts_rauzy, initial_scales=scales_rauzy, n_jitters=15)
