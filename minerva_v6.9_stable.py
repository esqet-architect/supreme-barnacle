"""
MINERVA V6.9: STABLE COCYCLE & SPECTRAL ANALYSIS
==================================================
- Fixed: ThueMorse D₂=0 issue (added minimum scale filtering)
- Fixed: SymPy import (removed unused factor_list which can hang)
- Fixed: Negative γ clamping to 0.0
- Preserved: Real Schur decomposition for stable subspace
- Preserved: v_i = e_i - f canonical centering
"""
...
def is_primitive_matrix(M, max_power=12):
    """Check if matrix is primitive (some power has all positive entries)."""
    M_k = np.copy(M)
...
def analyze_pisot_spectrum(evals):
    ...
    is_pisot = (dominant > 1.0) and (np.max(rest) < 0.99 if rest else True)
    return is_pisot, dominant, rest
...
def generate_sequence(sub_dict, seed, steps=12, max_len=50000):
    """Generate substitution sequence."""
    seq = seed
...
def compute_walk(seq, increments):
    """Compute walk from increment vectors."""
...
def project_via_schur(walk, M):
    """
    Project onto contracting subspace using Real Schur decomposition.
    Isolates stable coordinates by removing the Perron-Frobenius direction.
    """
    d = len(M)
    T, Z = schur(M, output='real')
    
    # Find dominant eigenvalue block (largest absolute diagonal)
    diag_abs = np.abs(np.diag(T))
    pf_block_idx = np.argmax(diag_abs)
    
    # Collect stable subspace columns
...
def compute_wandering_exponent(points):
    ...
    Clamps small negative values to 0.0 (finite-size artifact).
    """
    ...
    # Clamp micro-negative finite-size artifacts to zero
    if -0.05 < gamma < 0.0:
        gamma = 0.0
...
def compute_correlation_dimension(points, num_samples=600):
    """
    Estimate correlation dimension D₂ using Grassberger-Procaccia method.
    """
    n = len(points)
    if n < 50:
        return 0.0
    ...
    # Compute pairwise distances (optimized for memory)
    ...
    # Use median of local slopes for robustness
    slopes = []
    ...
    return max(0.0, min(d2, 2.0))  # Clamp to reasonable range
...
def main():
    print("=" * 75)
    print("MINERVA V6.9: STABLE COCYCLE & SPECTRAL ANALYSIS")
    ...
    for name, config in SYSTEMS.items():
        ...
        if is_pisot:
            ...
            print("   → Non-Pisot / Reducible System (diffuse or higher-dimensional)")
    ...
    csv_path = "minerva_v6.9_stable_metrics.csv"
    ...
    for r in results:
        if r["Pisot"]:
            if r["Subspace_Dim"] == 2 and r["Correlation_D2"] > 1.5:
                cls = "Rauzy fractal"
            elif r["Subspace_Dim"] == 1:
                cls = "1D interval exchange"
            else:
                cls = "Pisot attractor"
        else:
            cls = "Non-Pisot"
